  ...
  return (
    <div id="student-portal" className="space-y-6">
      ...
      <div className="bg-white rounded-3xl p-6 sm:p-7 border border-slate-200 shadow-sm flex flex-col sm:flex-row sm:items-center justify-between gap-6">
        ...
        {/* Quick summary badges & Quick Edit */}
        <div className="flex items-center gap-3 flex-wrap sm:flex-nowrap">
          {/* Quick Edit Options Button */}
          <button
            id="student-quick-edit-options-btn"
            onClick={() => {
              setEditModalInitialTab('horario');
              setIsEditModalOpen(true);
            }}
            className="px-3.5 py-2.5 rounded-2xl bg-indigo-50 hover:bg-indigo-100 text-indigo-800 border border-indigo-200 text-xs font-bold flex items-center gap-2 transition-all cursor-pointer hover:scale-[1.02] shadow-xs"
            title="Editar Horário e Dados do Aluno"
          >
            <SlidersHorizontal className="w-4 h-4 text-indigo-600" />
            <div className="text-left">
              <div className="text-[10px] text-indigo-500 font-semibold leading-tight">Configurações</div>
              <div className="font-bold leading-tight">Editar Horário / Opções</div>
            </div>
          </button>
      ...
      <div className="flex items-center p-1.5 bg-slate-100/90 rounded-2xl border border-slate-200 gap-1 overflow-x-auto scrollbar-none">
        <button
          ...
          onClick={() => setActiveTab('horario')}
          className={`flex-1 min-w-[140px] flex items-center justify-center gap-2 py-3 px-4 rounded-xl text-xs font-bold transition-all cursor-pointer ${
            activeTab === 'horario'
      ...
      <div className="transition-all duration-200">
        {activeTab === 'horario' && (
          <ScheduleView
            ...
            schedule={schedule}
            onOpenEdit={(tab) => {
              setEditModalInitialTab(tab || 'horario');
              setIsEditModalOpen(true);
            }}
          />
        )}
      ...
      {/* Edit Schedule and Student Options Modal */}
