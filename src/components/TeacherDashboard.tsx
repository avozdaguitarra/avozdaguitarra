...
export const TeacherDashboard: React.FC<TeacherDashboardProps> = ({
  ...
  const [newStudentDuration, setNewStudentDuration] = useState<number>(50);
  const [newStudentModality, setNewStudentModality] = useState<ModalityType>('presencial');
  const [newStudentRoom, setNewStudentRoom] = useState<string>('Estúdio 1');
    ...
    const newSchedule: ScheduleItem = {
      ...
      locationOrLink:
        newStudentModality === 'online'
          ? 'https://meet.google.com/aula-musica'
          : 'Rua da Música nº 42, Lisboa',
                    ...
                    <tr key={st.id} className="hover:bg-slate-50/70 transition-colors">
                      ...
                      <td className="py-4 pl-4 text-right">
                        <button
                          ...
                          className="px-3 py-1.5 bg-slate-100 hover:bg-indigo-50 hover:text-indigo-700 rounded-lg text-xs font-bold text-slate-700 transition-colors"
                        >
                          Ver Portal Aluno
                        </button>
                      </td>
